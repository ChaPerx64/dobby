...
export function CreateAllocationModal({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      // Currency Conversion: Convert to cents
      const amountInCents = Math.round(parsedAmount * 100)
      
      // Date Conversion: Convert to ISO string
      const isoDate = new Date(date).toISOString()

      const { error: apiError } = await apiClient.createTransaction({
        envelopeId,
        amount: amountInCents,
        description: description.trim() || "Funding",
        date: isoDate,
        category: "Funding",
      })

      if (apiError) {
        setError(apiError.message || "Failed to create allocation")
        return
      }
      ...
      <DialogContent className="sm:max-w-[425px]">
        <form onSubmit={handleSubmit}>
          <DialogHeader>
            ...
            <DialogDescription>
              Add funds to an envelope. This will increase its balance.
            </DialogDescription>
          </DialogHeader>
